...
export class RelativeVelocityFilter {
   ...
   * @param windowSize Higher windowSize adds to lag and to stability.
   * @param velocityScale Lower velocityScale adds to lag and to stability.
   */
  constructor(
  ...
  apply(value: number, macroSeconds: number, valueScale: number): number {
    ...
    if (this.lastTimestamp === -1) {
      ...
      alpha = 1 - 1 / (1 + this.velocityScale * Math.abs(velocity));
