...
export function tensorsToSegmentation(
    ...
    config: TensorsToSegmentationConfig): tf.Tensor2D {
  return tf.tidy(() => {
    ...
    if (tensorChannels === 1) {
      ...
      const outputMat = config.outputSize ?
          tf.image.resizeBilinear(
              smallMaskMat,
              [config.outputSize.height, config.outputSize.width]) :
          smallMaskMat;

      // Remove channel dimension.
      return tf.squeeze(outputMat, [2]) as tf.Tensor2D;
    } else {
      throw new Error(
