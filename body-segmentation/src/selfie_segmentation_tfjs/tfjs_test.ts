...
import {SelfieSegmentationTfjsModelConfig} from './types';

const MEDIAPIPE_MODEL_CONFIG: SelfieSegmentationTfjsModelConfig = {
  runtime: 'tfjs',
};
...
describeWithFlags('TFJS SelfieSegmentation static image ', BROWSER_ENVS, () => {
  ...
  beforeAll(async () => {
    ...
    segmentationImage = await loadImage('portrait_segmentation.png', 820, 1024);
  });
  ...
  it('test.', async () => {
    ...
    segmenter =
        await bodySegmentation.createSegmenter(model, MEDIAPIPE_MODEL_CONFIG);
