...
import {DEFAULT_SELFIESEGMENTATION_MODEL_CONFIG, DEFAULT_SELFIESEGMENTATION_MODEL_URL_GENERAL, DEFAULT_SELFIESEGMENTATION_MODEL_URL_LANDSCAPE, DEFAULT_SELFIESEGMENTATION_SEGMENTATION_CONFIG} from './constants';
import {SelfieSegmentationTfjsModelConfig, SelfieSegmentationTfjsSegmentationConfig} from './types';

export function validateModelConfig(
    modelConfig: SelfieSegmentationTfjsModelConfig):
    SelfieSegmentationTfjsModelConfig {
  if (modelConfig == null) {
    return {...DEFAULT_SELFIESEGMENTATION_MODEL_CONFIG};
  }

  const config: SelfieSegmentationTfjsModelConfig = {...modelConfig};
  ...
  if (config.modelUrl == null) {
    switch (config.modelType) {
      case 'general':
        config.modelUrl = DEFAULT_SELFIESEGMENTATION_MODEL_URL_GENERAL;
        break;
      case 'landscape':
      default:
        config.modelUrl = DEFAULT_SELFIESEGMENTATION_MODEL_URL_LANDSCAPE;
        break;
    }
  }
