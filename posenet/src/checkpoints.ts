...
export function resNet50Checkpoint(stride: number, quantBytes: number): string {
  const graphJson = `resnet50-stride${stride}/model.json`;
...
export function mobileNetCheckpoint(
  ...
  const graphJson = `mobilenet-${toStr[multiplier]}-stride${stride}/model.json`;
  // quantBytes=4 corresponding to the non-quantized full-precision checkpoints.
  if (quantBytes == 4) {
    return MOBILENET_BASE_URL + `float/` + graphJson;
  } else {
    return MOBILENET_BASE_URL + `quant${quantBytes}/` + graphJson;
  }
}
