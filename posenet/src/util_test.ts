...
import {PoseNetInternalResolution, PoseNetOutputStride} from './types';
...
describeWithFlags('util.toInputResolutionHeightAndWidth', ALL_ENVS, () => {
  function getExpectedResolution(
      inputShape: [number, number], outputStride: PoseNetOutputStride,
  ...
  it(`returns the 50% of the image size as a valid input ` +
     ...
     () => {
       const inputShape: [number, number] = [1450, 789];
       const outputStride = 16;
       const internalResolution: PoseNetInternalResolution = undefined;
       const expectedScalePercentage = 0.50;
