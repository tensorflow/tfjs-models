...
export function drawSkeleton(keypoints, minConfidence, ctx, scale = 1) {
    ...
    adjacentKeyPoints.forEach((keypoints) => {
        drawSegment(toTuple(keypoints[0].point), toTuple(keypoints[1].point), '#0000ff', scale, ctx);
    });
}
...
export function drawOffsetVectors(heatMapValues, offsets, outputStride, scale = 1, ctx) {
    ...
    for (let i = 0; i < heatmapData.length; i += 2) {
        ...
        drawSegment([heatmapY, heatmapX], [offsetPointY, offsetPointX], '#ffff00', scale, ctx);
    }
}
