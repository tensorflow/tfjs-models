...
export function drawHeatmapImage(heatmaps) {
    ...
    const scaledUp = posenet.resizeBilinearGrayscale(singleChannelImage, [100, 100]);
...
function drawHeatmapAsAlpha(image, heatmaps, outputStride, canvas) {
    ...
    const alphadImage = posenet.setHeatmapAsAlphaChannel(pixels, outputStride, singleChannelImage);
    renderToCanvas(alphadImage, canvas);
}
function drawResults(image, heatmaps, outputStride, poses, minPartConfidence, minPoseConfidence) {
    ...
    poses.forEach(pose => {
        if (pose.score >= minPoseConfidence) {
            drawKeypoints(pose.keypoints, minPartConfidence, resultsCanvas.getContext('2d'));
            drawSkeleton(pose.keypoints, minPartConfidence, resultsCanvas.getContext('2d'));
        }
    });
}
...
async function testImageForSinglePoseAndDrawResults(model, imagePath, guiState) {
    const image = await loadImage(imagePath);
    const pixels = tf.fromPixels(image);
    const { heatmapScores, offsets } = model.predictForSinglePose(pixels, guiState.outputStride);
    const pose = posenet.singlePose.decode(heatmapScores, offsets, guiState.outputStride);
    drawResults(image, heatmapScores, guiState.outputStride, [pose], guiState.minPartConfidence, guiState.minPoseConfidence);
}
async function testImageForMultiplePosesAndDrawResults(model, imagePath, guiState) {
    const image = await loadImage(imagePath);
    const pixels = tf.fromPixels(image);
    const { heatmapScores, offsets, displacementBwd, displacementFwd } = model.predictForMultiPose(pixels, guiState.outputStride);
    const poses = await posenet.multiPose.decode(heatmapScores, offsets, displacementFwd, displacementBwd, guiState.outputStride, guiState.multiPoseDetection.maxDetections, guiState.minPartConfidence, guiState.multiPoseDetection.nmsRadius);
    drawResults(image, heatmapScores, guiState.outputStride, poses, guiState.minPartConfidence, guiState.minPoseConfidence);
}
...
function setupGui(model) {
    const guiState = {
        ...
        singlePoseDetection: {
            detect: () => {
                testImageForSinglePoseClick(model, `/images/${guiState.image}`, guiState);
            }
        },
        multiPoseDetection: {
            detect: () => {
                testImageForMultiPoseClick(model, `/images/${guiState.image}`, guiState);
            },
            nmsRadius: 13.0,
            maxDetections: 15
        }
